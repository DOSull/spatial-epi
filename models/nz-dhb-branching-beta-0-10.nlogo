...
globals [
  ;; list of all cases in order of first exposure in their queue
  ;; a list of [t case] pairs where each case has _multiple_
    ...
    [
      repeat initial-infected [
        create-subclinical-cases 1 [
          ...
          initialise-case t p-clinical true myself
        ]
      ]
    ]
  ]
...
to update-parameters
  ...
  set total-infected total-cases + count subclinical-cases
...
to-report total-burden-remaining
  report count clinical-cases +
         count subclinical-cases +
         length exposures-to-come
end
...
;; ------------------------------------
to-report next-exposure
  report first exposures
end


to-report exposures-as-t-case-pairs
  report map [ x -> (list x self) ] exposures
end
...
to-report insert-tx-in-order [L t-x-pair]
  report (sentence filter [ t -> (first t) < (first t-x-pair) ] L
                   (list t-x-pair)
                   filter [ t -> (first t) >= (first t-x-pair) ] L)
end
...
to-report split-cases-at-time [L t] ;; assume list is ordered
...
to progress-cases [time-now]
  ask (turtle-set clinical-cases subclinical-cases) [
    let time-since-infection time-now - t-0
    if time-since-infection > 30 [ ;; this is weirdly high 'for computational convenience' but this makes no sense to me
      ask my-locale [
        set new-recovered new-recovered + 1
      ]
      set R-effs lput infections-caused R-effs
      set R-eff mean R-effs
      die
    ]
  ]
  set exposures-to-come filter [ t-x -> item 1 t-x != nobody ] exposures-to-come
...
to-report my-cluster
  ...
  let next-generation (turtle-set [out-transmission-neighbors] of self)
  while [any? next-generation] [
    set cluster (turtle-set cluster next-generation)
    set next-generation (turtle-set [out-transmission-neighbors] of next-generation)
  ]
  report cluster
end
...
;; i.e. all exposures it will cause to be tested
;; time to onset, time to isolation, hospitalisation, recovery, death
to initialise-case [t p-clin old-case? cause]
  set breed subclinical-cases

  ;; common stuff
  set t-0 t
  set t-recover 30 ;; 'for computational convenience'! will experiment with varying this later
  ...
  ifelse random-float 1 < p-clin [       ;; TPM paper
...
to initialise-clinical-case
  ...
  set t-isolation t-0 + t-onset-1 + t-onset-2
  set t-enter-hospital t-0 + t-onset-1
  set hospitalised? random-float 1 < 0.078
...
to initialise-exposures
  set exposures n-values random-poisson base-R [t -> t-0 + random-weibull 2.83 5.67 0 t-recover]
end
...
to add-exposures-to-old-case
  ...
  [
    ...
    set exposures sort n-values n [t -> t-0 + random-weibull 2.83 5.67 (0 - t-0) 30]
  ]
end
...
68
R-eff
mean [infections-caused] of ((turtle-set clinical-cases subclinical-cases) with [infections-caused > 0])
3
1
11
...
1349
66
Note that due to isolation \nR-eff is never as high as \nR-mean
12
0.0
1
