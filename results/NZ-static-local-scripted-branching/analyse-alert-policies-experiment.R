...
setwd("~/Documents/code/spatial-epi/results/NZ-static-local-scripted-branching")
...
ggplot(dhb.totals, aes(x=t)) +
  ...
  ggtitle('Branching model, NZ DHBs, 2000 random cases')
...
ggplot(ta.totals, aes(x=t)) +
  ...
  xlab('Days') +
  ylab('New symptomatic cases per day') + 
  ggtitle('Branching model, NZ TAs, 2000 random cases')

ggsave('branching-nz-tas-new-cases-time-series-by-alert-policy.png')
...
ggsave('branching-nz-tas-population-in-alert-levels-by-alert-policy.png')
...
ggplot(eradication.times, aes(x=alert.policy)) +
  ...
  ggtitle('Branching model, 2000 random cases')

ggsave('branching-nz-eradication-times.png')
